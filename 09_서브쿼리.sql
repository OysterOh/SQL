...
WHERE   salary > ANY (SELECT  salary
                            FROM  employees
                            WHERE  first_name = 'David'); -- 최소값:5800 (>4800)
...
-- ROWNUM이 정렬되지 않는 상황이 발생한다
-- 이유: ROWNUM이 먼저 붙고 정렬이 진행되기 때문 ORDER BY는 항상 마지막에 진행
-- 해결: 정렬이 미리 진행된 자료에 ROWNUM을 붙여서 다시 조회한다
...
-- 이유: WHERE절 먼저 실행하고나서 ROWNUM이 SELECT되기 때문
-- 해결: ROWNUM까지 붙여놓고 다시 한 번 자료를 SELECT 해서 범위를 지정해야한다
...
SELECT
    ...
    SELECT '오정원', '20230501' FROM dual UNION ALL
    SELECT '원지윤', '20230601' FROM dual UNION ALL
    SELECT '김테스', '20230701' FROM dual
    )
)
    WHERE mm = '0418';