...
트리거를 만들 때 범위 지정하고 F5버튼으로 부분실행해야한다
...
CREATE OR REPLACE TRIGGER trg_test
    ...
    FOR EACH ROW  -- 각 행에 모두 적용(생략가능) 생략 시 한번만 실행
-- DECLARE 생략 가능
BEGIN 
    dbms_output.put_line('트리거가 동작함');  -- 실행하고자 하는 코드를 begin ~ end에 넣는다
END;


INSERT INTO tbl_test VALUES(1, '김철수');   -- 동작 X
