...
view는 기본 테이블로 유도된 가상 테이블이기 때문에
필요한 컬럼만 저장해두면 관리가 용이하다
view는 가상테이블로 실제 데이터가 물리적으로 저장된 형태는 아니다
view를 통해서 데이터에 접근하면 원본 데이터는 안전하게 보호될 수 있다
*/
...
GROUP BY job_title
ORDER BY AVG(salary) DESC;  -- SQL 구문이 짧아졌다
...
-- 두 번째 컬럼인 'name'은 가상열(virtual column)이기 때문에 INSERT가 안된다
...
-- 읽기 전용 뷰 -> WITH READ ONLY(DML 연산을 막는다 -> SELECT만 허용)
