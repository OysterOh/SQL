...
VALUES
    (105, '데이비드', '베컴', 'DAVIDBECKHAM', '23/04/19', 'IT_PROG');
SELECT * FROM emps_it;
...
WHEN MATCHED THEN  -- 조건에 일치하는 경우 타겟 테이블에 이렇게 실행하라
    UPDATE SET 
        ...
        UPDATE된 대상을 DELETE하도록 설계되어 있으므로
...
WHEN NOT MATCHED THEN -- 조건에 일치하지 않는 경우 타겟 테이블에 이렇게 실행하라
    INSERT  /*속성(컬럼1 )*/ VALUES
...
/*
employees 테이블을 매번 빈번하게 수정되는 테이블이라고 가정한다
기존의 데이터는 email, phone, salary, commission_pct, manager_id, department_id을
업데이트 하도록 처리
새로 유입된 데이터는 그대로 추가
*/
...
-- 2-3
UPDATE depts SET 
    department_name = 'IT_HELP', manager_id = 303, location_id = 1800
