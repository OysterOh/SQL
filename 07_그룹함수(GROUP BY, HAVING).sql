...
SELECT
    ...
    AVG(salary) AS 월급평균
FROM employees
WHERE department_id >= 50  --FROM 다음 WHERE
GROUP BY department_id
...
SELECT job_id, AVG(salary), COUNT(*) AS 사원수
FROM employees
GROUP BY job_id
...
-- TO_CHAR 함수 사용해서 연도만 변환한다 그것을 그룹화

--SELECT  TRUNC((sysdate - hire_date)/365, 0)
--FROM employees
--GROUP BY TRUNC((sysdate - hire_date)/365, 0);

SELECT TO_CHAR(hire_date, 'yy'), COUNT(*)
FROM employees
GROUP BY TO_CHAR(hire_date, 'yy')
ORDER BY TO_CHAR(hire_date, 'yy');  --짬순
...
WHERE salary >= 5000
GROUP BY department_id
HAVING AVG(salary) >= 7000; -- O

-- 문제 4.
-- 사원 테이블에서 commission_pct(커미션) 컬럼이 null이 아닌 사람들의
-- department_id(부서별) salary(월급)의 평균, 합계, count를 구합니다.
-- 조건 1) 월급의 평균은 커미션을 적용시킨 월급입니다.
-- 조건 2) 평균은 소수 둘째 자리에서 절삭 하세요
SELECT
    department_id,
