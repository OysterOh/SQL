...
-- OUT을 이용해서 보낸 값은 바깥 익명 블록에서 실행해야 한다.
...
IS
BEGIN
    dbms_output.put_line('p_var1: ' || p_var1);  -- 당연히 된다
    dbms_output.put_line('p_var2: ' || p_var2);  -- 값이 전달되지 않았다(공백)
    dbms_output.put_line('p_var3: ' || p_var3);  -- IN의 성질을 가진다
    
   --  p_var1 := '결과1';  IN 변수는 값 할당 자체가 불가하다
   --  PLS-00363: expression 'P_VAR1' cannot be used as an assignment target
    p_var2 := '결과2';
    p_var3 := '결과3';
END;
