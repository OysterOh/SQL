...
SELECT 컬럼리스트 FROM 조인 대상이 되는 테이블 (1개이상)
WHERE 조인 조건 -> 오라클 조인
*/
...
SELECT 
    first_name, last_name, hire_date, salary, job_id, e.department_id, department_name
FROM employees e, departments d
WHERE e.department_id = d.department_id;  --오라클 조인

SELECT
    e.first_name, e.last_name, e.hire_date, e.salary, e.job_id, e.department_id, d.department_name
FROM employees e
...
테이블 이름을 작성하여 소속을 표현하는것이 바람직하다
테이블 이름이 너무 길 시 ALIAS를 작성하여 칭한다
두 테이블 모두 가지고 있는 컬럼의 경우 반드시 명시해야 한다
*/

-- 3개의 테이블을 이용한 내부 조인 (INNER JOIN)
-- 내부 조인: 두 테이블 모두에서 일치하는 값을 가진 행만 반환한다
SELECT
...
/*
외부 조인 진행시 모든 조건에 (+)를 붙여야 외부 조인이 유지된다
