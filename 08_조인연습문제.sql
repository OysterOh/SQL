...
SELECT *
FROM employees e, departments d
WHERE e.department_id = d.department_id(+); --null 포함?? 107
...
SELECT e.first_name ||' '|| e.last_name AS name, d.department_id
FROM employees e
INNER JOIN departments d
...
--문제 4.
----JOBS테이블과 JOB_HISTORY테이블을 LEFT OUTER JOIN 하세요.
SELECT *
FROM jobs j
LEFT OUTER JOIN job_history
ON j.job_id = job_history.job_id;
...
SELECT e1.manager_id, e1.first_name, e1.manager_id, e2.first_name AS manager_id, e2.salary
FROM employees e1
