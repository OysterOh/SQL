...
FROM
    dual; 
-- dual: 일시적인 연산의 결과값에 사용
/*
dual이라는 테이블은 sys가 소유하는 오라클의 표준 테이블로서, 
오직 한 행에 한 컬럼만 담고있는 dummy테이블이다
일시적인 산술 연산이나 날짜 연산 등에 주로 사용한다
모든 사용자가 접근할 수 있다
*/
...
SELECT LTRIM('javascript_java', 'java')     -- script_java
FROM dual;

SELECT RTRIM('javascript_java', 'java')     -- javascript_
FROM dual;

SELECT TRIM('        java         ') FROM dual; -- java

--SELECT TRIM('javascript_java', 'java') FROM dual; (X)
...
SELECT 
    LOWER(RPAD(SUBSTR(first_name,1,3), LENGTH(first_name), '*')) AS name, 
